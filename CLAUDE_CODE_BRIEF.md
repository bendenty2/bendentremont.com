...
│            all imgs+vids) │  resizes,     │ videos/      (copied as-is)    │
...
- `script.js` — all behavior (vanilla JS, one IIFE). Responsibilities: Pics/About view
  ...
  via the pattern engine (§5) **and the separate `renderGridMobile` at ≤700px**, the lightbox
  (open / prev-next / keyboard / hover-preload — videos are **muted, no audio**), and evenly
  distributes videos through the grid.
...
(`object-fit`/scaleX trick in CSS), never letterboxing. They autoplay muted and loop
everywhere — **there is no audio** (the lightbox unmute toggle was removed 2026-06-20).
